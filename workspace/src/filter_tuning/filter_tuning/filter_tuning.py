...
class FilterTuning(Node):
    def __init__(self):
        ...
        self.pose_file = open(self.path + 'pose.csv', 'w')
        self.ctrl_file = open(self.path + 'ctrl.csv', 'w')
    ...
    def odom_raw_callback(self, msg):
        self.odom_raw_file.write(f'{msg.pose.pose.position.x},{msg.pose.pose.position.y},{self.get_yaw_from_quaternion(msg.pose.pose.orientation)}')

    def pose_stamped_callback(self, msg):
        self.pose_file.write(
            f'{msg.pose.position.x},{msg.pose.position.y},{self.get_yaw_from_quaternion(msg.pose.orientation)}')
