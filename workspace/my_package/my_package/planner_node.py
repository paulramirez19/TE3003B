import heapq
import rclpy
from rclpy.node import Node # Keep this for the ROS 2 node base class
...
class GridNode: # Renamed from Node to GridNode
    """
    ...
    def __lt__(self, other):
        ...
        GridNodes with lower estimated_total_cost are prioritized.
        """
...
class DStarPlanner:
    ...
    def plan_path(self, start_coord, goal_coord):
        ...
        start_node = GridNode(start_grid[0], start_grid[1], 0, self.heuristic(start_grid, goal_grid)) # Changed to GridNode
        goal_node_coords = (goal_grid[0], goal_grid[1])

        # Priority queue for GridNodes to be evaluated, ordered by estimated_total_cost
        open_set = [start_node]
        # Set to store already evaluated GridNodes (grid coordinates)
        closed_set = set()
        # Dictionary to store the actual cost (g-score) to a GridNode, for efficient updates
        ...
        while open_set:
            ...
            for neighbor in self.get_neighbors(current_node):
                ...
                if (neighbor_x, neighbor_y) not in g_scores or new_cost < g_scores[(neighbor_x, neighbor_y)]:
                    ...
                    neighbor_node = GridNode(neighbor_x, neighbor_y, new_cost, neighbor_heuristic) # Changed to GridNode
